...
def pageRankAmeliore(P, damping_factor, tolerance):
...
def simulate_epidemic_pagerank_vaccinated(G, nb_infected, nb_vaccinated, infection_rate, recovery_rate,
    ...
    # aléatoirement
    #vaccinated_position = np.random.choice(vaccinated.size, nb_infected, replace=False)
    #vaccinated[vaccinated_position] = 1
...
if __name__ == '__main__':
    ...
    graph = GraphHandler(filename)
    nodes_nb = graph.get_nodes_nb()

    nb_infected = int(nodes_nb * 0.3)
    # Nombre n d'individus vaccinés au départ (10% de la population)
    nb_vaccinated = math.ceil(nodes_nb * 0.3)

    temps_execution = timeit.timeit(str(simulate_epidemic_pagerank_vaccinated(graph, nb_infected, nb_vaccinated,
                                    infection_rate, recovery_rate, vaccination_rate)), number=20)
